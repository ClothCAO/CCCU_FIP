...
app.post('/saveArticle', async(req, res)=>{
	...
	var number = 'SELECT COUNT(*) FROM article' 
	var index;
	var num = db.query(number,function(err, results, fields){
		if(err) 
		{
			console.log(err);
	...
	index = num
	var insert = 'INSERT INTO article (articleID, title, eventID, salesID) VALUES (?, ?, ?, ?)';
	var info = [index,req.body.name, req.body.eventID, req.body.salesID];
