    ...
    <div class=" w3-container">
      <ul>
        <li v-if="hvArt" v-for="art in artList" class="w3-bar w3-border-bottom">
          <span class="w3-bar-item"><i v-if="art.status == 1" class="material-icons">language</i><i v-if="art.status == 0" class="material-icons">mode_edit</i></span>
          <span v-if="edit==true && art.status == 0" class="w3-btn w3-bar-item w3-right w3-text-red"><i class="material-icons">remove_circle</i></span>
          <div class="w3-bar-item">
            <span class="w3-large">{{art.title}}</span>
          </div>
        </li>
      </ul>
    </div>
